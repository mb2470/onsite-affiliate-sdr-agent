-- Fix foreign keys that block prospect (formerly lead) deletion.
--
-- activity_log.lead_id and outreach_log.lead_id reference prospects(id)
-- but were created WITHOUT ON DELETE CASCADE, so deleting a prospect that
...
-- outreach_log: CASCADE on delete (remove outreach rows when prospect is deleted)

-- 1. activity_log — set lead_id to NULL when the prospect is deleted
...
-- 2. outreach_log — cascade delete when the prospect is deleted
